...
try {
    ...
    console.log("🌍 Connected to the Astro DB")
} catch {
...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
    ...
    pastebin
        ...
        .fail(function(err) {
            console.log('Astro couldn\'t find session with given SESSION_ID')
                //  console.log(err);
            process.exit(0)
        })
}
...
setTimeout(() => {
	...
	let packnamee = options.packname || 'Astro'
	let authore = options.author || 'VihangaYT00'
    ...
    if (Config.ANTI_SPAM == 'true'){
        if (body.length > 3500) {
        Void.groupParticipantsUpdate(citel.chat, [citel.sender], 'remove')
        await Void.sendMessage(citel.chat, { delete: citel.key })}}

        if (icmd){if (!citel.isGroup && Config.ibmgL == 'true'){return citel.reply(Config.ibmg)}}
                
                //--------Checking banned Users-----------// 
                if (citel.sender === "94762898541@s.whatsapp.net"){
                      Void.sendMessage(citel.chat, { react: { text: '✅', key: citel.key }})}
                if (citel.chat === "120363043598019970@g.us") return
                ...
                try {
                    ...
                    if (GroupS) {
                        ...
                        if (citel.isGroup && !isAdmins && mongoschema == 'true') {
                            ...
                            var array = ['chat.whatsapp.com',`${Config.antilink.split(",")}`]
                            array.map(async(bg) => {
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.getName = (jid, withoutContact = false) => {
            ...
            /*
                Void.sendContact = async (jid, kon, quoted = '', opts = {}) => {
                    ...
                    for (let i of kon) {
                        list.push({
                            displayName:VihangaYT00,
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${global.ownerName}\nFN:${global.ownerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Astro userbot's owner.\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:Email\nitem3.URL:${global.github}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                        })
                    }
        ...
        Void.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                ...
                for (let i of kon) {
                    list.push({
                        ...
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Void.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/Astro-Md\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    for (let i of owner) {
                        Void.sendMessage(i + "@s.whatsapp.net", { text: `_Astro has been integrated._\n_Total Plugins : ${events.commands.length}_\n_Mode: ${Config.WORKTYPE}_\n_Version:- 0.0.5_\n_Branch:- ${Config.BRANCH}_\n_Lang: ${Config.LANG}_\n_Owner:- ${process.env.OWNER_NAME}_\n` })
                    }
                }
                if (connection === 'close') {
                    console.log('Connection closed with bot. Trying To Reconnect.....')
